...
bool onSeg(const Line &l, const Point &p) { // 点在线段上
	return sgn(det(p - l.a, l.b - l.a)) == 0 && sgn(dot(p - l.a, p - l.b)) <= 0;
}
...
double disToSeg(const Line &l, const Point &p) { // 点到线段距离
	return sgn(dot(p - l.a, l.b - l.a)) * sgn(dot(p - l.b, l.a - l.b)) != 1 ?
...
bool isCC(Circle a, Circle b, Point &p1, Point &p2) { 
	...
	if (sgn(s1 - a.r - b.r) > 0 || sgn(s1 - abs(a.r - b.r)) < 0) return false;
...
vector<Point> convexCut(const vector<Point>&ps, Line l) { // 用半平面 (q1,q2) 的逆时针方向去切凸多边形
	...
	for (int i = 0; i < n; ++i) {
		...
		int d1 = sgn(det(l.a, l.b, p1)), d2 = sgn(det(l.a, l.b, p2));
...
vector<Point> convexHull(vector<Point> ps) { // 求点集 ps 组成的凸包
	...
	for (int i = 0; i < n; qs.push_back(ps[i++])) 
		while (qs.size() > 1 && sgn(det(qs[qs.size()-2],qs.back(),ps[i])) <= 0) qs.pop_back();
	for (int i = n - 2, t = qs.size(); i >= 0; qs.push_back(ps[i--])) 
		while ((int)qs.size() > t && sgn(det(qs[(int)qs.size()-2],qs.back(),ps[i])) <= 0) qs.pop_back();
